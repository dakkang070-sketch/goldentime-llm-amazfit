...
export const Settings: React.FC = () => {
  ...
  const [settings, setSettings] = useState<SystemSettings>({
    transmissionInterval: { realtime: '권장 (10초)', healthStats: '5분' },
    thresholds: {
      highHeartRate: 120,
      lowHeartRate: 45,
      lowOxygen: 90,
      highTemperature: 38.0,
      fallSensitivity: '보통 (권장)'
    },
    privacy: {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">설정 로딩 중...</div>;
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button 
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors shadow-sm text-[14px] font-light disabled:opacity-50"
          >
            ...
            <SettingSection title="데이터 전송 주기 설정 (기본 정책)" icon={Clock}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  ...
                  <select 
                    value={settings.transmissionInterval.realtime}
                    onChange={(e) => handleChange('transmissionInterval', 'realtime', e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[15px] text-black focus:outline-none focus:border-blue-500 font-light"
                  >
                ...
                <div>
                  ...
                  <select 
                    value={settings.transmissionInterval.healthStats}
                    onChange={(e) => handleChange('transmissionInterval', 'healthStats', e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[15px] text-black focus:outline-none focus:border-blue-500 font-light"
                  >
                    <option>1분</option>
                    <option>5분</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                ...
                >
                   <div className="flex items-center gap-2">
                     <input 
                       type="number" 
                       value={settings.thresholds.highHeartRate}
                       onChange={(e) => handleChange('thresholds', 'highHeartRate', Number(e.target.value))}
                       className="w-full border border-slate-300 rounded-lg px-3 py-2 bg-white text-[15px] text-black focus:outline-none focus:border-blue-500 font-light transition-colors" 
                     />
                ...
                >
                   <div className="flex items-center gap-2">
                     <input 
                       type="number" 
                       value={settings.thresholds.lowHeartRate}
                       onChange={(e) => handleChange('thresholds', 'lowHeartRate', Number(e.target.value))}
                       className="w-full border border-slate-300 rounded-lg px-3 py-2 bg-white text-[15px] text-black focus:outline-none focus:border-blue-500 font-light transition-colors" 
                     />
                ...
                >
                   <div className="flex items-center gap-2">
                     <input 
                       type="number" 
                       value={settings.thresholds.lowOxygen}
                       onChange={(e) => handleChange('thresholds', 'lowOxygen', Number(e.target.value))}
                       className="w-full border border-slate-300 rounded-lg px-3 py-2 bg-white text-[15px] text-black focus:outline-none focus:border-blue-500 font-light transition-colors" 
                     />
                     <span className="text-black text-[13px] font-light w-8">이하</span>
                   </div>
                ...
                >
                   <div className="flex items-center gap-2">
                     <input 
                       type="number" 
                       step="0.1" 
                       value={settings.thresholds.highTemperature}
                       onChange={(e) => handleChange('thresholds', 'highTemperature', Number(e.target.value))}
                       className="w-full border border-slate-300 rounded-lg px-3 py-2 bg-white text-[15px] text-black focus:outline-none focus:border-blue-500 font-light transition-colors" 
                     />
                     <span className="text-black text-[13px] font-light w-8">이상</span>
                   </div>
                ...
                >
                   <select 
                     value={settings.thresholds.fallSensitivity}
                     onChange={(e) => handleChange('thresholds', 'fallSensitivity', e.target.value)}
                     className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-[15px] text-black focus:outline-none focus:border-blue-500 font-light transition-colors cursor-pointer"
                   >
                    <option>높음 (민감)</option>
                    <option>보통 (권장)</option>
                    <option>낮음 (둔감)</option>
            ...
            <SettingSection title="권한 및 개인정보 (시스템 기본값)" icon={Shield}>
               <div className="space-y-6">
                 <div className="flex justify-between items-center cursor-pointer" onClick={() => handleToggle('locationCollection')}>
                   ...
                   <Toggle checked={settings.privacy.locationCollection} onClick={() => handleToggle('locationCollection')} />
                 </div>
                 ...
                 <div className="flex justify-between items-center cursor-pointer" onClick={() => handleToggle('dataAnalysis')}>
                   ...
                   <Toggle checked={settings.privacy.dataAnalysis} onClick={() => handleToggle('dataAnalysis')} />
                 </div>
                 ...
                 <div className="flex justify-between items-center cursor-pointer" onClick={() => handleToggle('autoReport')}>
                   ...
                   <Toggle checked={settings.privacy.autoReport} onClick={() => handleToggle('autoReport')} />
                 </div>
               </div>
            </SettingSection>
        </div>
