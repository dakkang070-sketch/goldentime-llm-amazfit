...
export const Members: React.FC = () => {
  ...
  const handleHealthSave = () => {
    // Health info is read-only from sensors usually, but if manual edit is needed:
    // This part might need a separate API or just local mock for now as backend doesn't support manual health edit yet
    if (selectedMember && editForm) {
                    ...
                    <td className="p-4">
                      {(() => {
                        if (member.accountStatus === 'suspended') {
                          return <span className="px-2.5 py-1 rounded-full text-[13px] border whitespace-nowrap bg-amber-50 text-amber-600 border-amber-200">이용 정지</span>;
                        }
                        if (member.accountStatus === 'withdrawn') {
                          return <span className="px-2.5 py-1 rounded-full text-[13px] border whitespace-nowrap bg-slate-200 text-slate-600 border-slate-300">해지</span>;
                        }
                        return (
                          <span className={`px-2.5 py-1 rounded-full text-[13px] border whitespace-nowrap ${
                            ...
                            member.status === '주의' ? 'bg-orange-50 text-orange-600 border-orange-200' :
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm relative">
                       <div className="flex justify-between items-start mb-3">
                         ...
                         {!isEditingPersonal ? (
                           <button onClick={() => setIsEditingPersonal(true)} className="p-1 text-black hover:text-blue-600 transition-colors">
                             <Pencil size={12} />
                           </button>
                         ) : (
                           <div className="flex gap-1">
                             <button onClick={handlePersonalInfoSave} className="p-1 bg-green-50 text-green-600 rounded hover:bg-green-100">
                               <Check size={12} />
                             </button>
                             <button onClick={() => setIsEditingPersonal(false)} className="p-1 bg-red-50 text-red-500 rounded hover:bg-red-100">
                               <X size={12} />
                             </button>
                           </div>
                         )}
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm relative">
                       <div className="flex justify-between items-start mb-3">
                         ...
                         {!isEditingAccount ? (
                           <button onClick={() => setIsEditingAccount(true)} className="p-1 text-black hover:text-blue-600 transition-colors">
                             <Pencil size={12} />
                           </button>
                         ) : (
                           <div className="flex gap-1">
                             <button onClick={handleAccountStatusSave} className="p-1 bg-green-50 text-green-600 rounded hover:bg-green-100">
                               <Check size={12} />
                             </button>
                             <button onClick={() => setIsEditingAccount(false)} className="p-1 bg-red-50 text-red-500 rounded hover:bg-red-100">
                               <X size={12} />
                             </button>
                           </div>
                         )}
                       ...
                       {isEditingAccount ? (
                         ...
                         <div className="space-y-1">
                           <InfoItem icon={Shield} label="계정 상태" value={
                             selectedMember.accountStatus === 'active' ? '이용 중' :
                             selectedMember.accountStatus === 'suspended' ? '이용 정지' :
                             selectedMember.accountStatus === 'withdrawn' ? '해지' : '이용 중'
                           } />
                         </div>
                       )}
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm relative">
                       <div className="flex justify-between items-start mb-3">
                         ...
                         {!isEditingPassword ? (
                           <button onClick={() => setIsEditingPassword(true)} className="p-1 text-black hover:text-blue-600 transition-colors">
                             <Pencil size={12} />
                           </button>
                         ) : (
                           <div className="flex gap-1">
                             <button onClick={handlePasswordSave} className="p-1 bg-green-50 text-green-600 rounded hover:bg-green-100">
                               <Check size={12} />
                             </button>
                             <button onClick={() => setIsEditingPassword(false)} className="p-1 bg-red-50 text-red-500 rounded hover:bg-red-100">
                               <X size={12} />
                             </button>
                           </div>
                         )}
