...
import { CalendarDays, CheckCircle2, Mail, MapPin, Phone, Plus, RefreshCw, Search, ShieldCheck, Trash2, UserCircle2, X, XCircle } from 'lucide-react';
...
const controllerFormSelectClass = 'h-11 w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-[15px] text-black outline-none focus:border-blue-500';
const controllerEmailDomainGridClass = 'mt-2 grid grid-cols-1 gap-2 md:grid-cols-[minmax(0,1fr)_20px_minmax(220px,240px)] md:items-center';

/**
 * 관제요원 계정 상태를 회원관리와 유사한 배지 스타일로 변환합니다.
 */
...
/**
 * 관제요원 전체 계정 리스트와 상세/승인 처리를 제공하는 관리자 페이지입니다.
 */
export const ControllerManagement: React.FC = () => {
  ...
  const [controllers, setControllers] = useState<PendingStaffApproval[]>([]);
    ...
    createPasswordConfirm.length === 0
      ...
      : createForm.password.length < 8
        ...
        : createForm.password === createPasswordConfirm
          ? '비밀번호가 일치합니다.'
          : '비밀번호가 일치하지 않습니다.';
  const isCreateControllerFormValid =
    createForm.name.trim().length >= 2 &&
    createPhoneRest.length === 8 &&
  ...
  /**
   * 관제요원 전체 계정 목록을 불러옵니다.
   */
  const fetchControllers = async () => {
    try {
      setLoading(true);
      const staff = await adminService.getStaffAccounts();
      const next = staff.filter((entry) => entry.role === 'controller');
      setControllers(next);
  ...
  /**
   * 현재 선택된 관제요원 상세 정보를 계산합니다.
   */
  const selectedController = useMemo(
    () => controllers.find((entry) => entry.id === selectedControllerId) || null,
    [controllers, selectedControllerId],
  );

  /**
   * 선택된 관제요원이 바뀌면 우측 계정 상태 드래프트도 함께 동기화합니다.
   */
  useEffect(() => {
    if (!selectedController) return;
    const nextPhone = splitPhoneInput(selectedController.phone || '');
    const nextEmail = splitEmailInput(selectedController.email || '');
    ...
    setAccountStatusDraft(selectedController.accountStatus);
    setEditForm({
      email: selectedController.email || '',
      phone: formatPhoneNumber(selectedController.phone || ''),
      role: selectedController.role,
      city: selectedController.affiliation.city || '',
      district: selectedController.affiliation.district || '',
      dong: selectedController.affiliation.dong || '',
    });
  }, [selectedController]);
  const editPhoneDisplayValue =
    editPhoneRest.length <= 4 ? editPhoneRest : `${editPhoneRest.slice(0, 4)}-${editPhoneRest.slice(4, 8)}`;
  const editResolvedEmailDomain =
    editEmailDomain === EMAIL_DOMAIN_DIRECT_VALUE ? editEmailCustomDomain.trim() : editEmailDomain;
  const normalizedEditEmail = editForm.email.trim();
  const normalizedSelectedControllerPhone = formatPhoneNumber(selectedController?.phone || '');
  ...
  const hasEditEmailChanged = Boolean(selectedController) && normalizedEditEmail !== (selectedController?.email || '');
  const hasEditPhoneChanged = Boolean(selectedController) && normalizedEditPhone !== normalizedSelectedControllerPhone;
  const hasEditRegionChanged =
    Boolean(selectedController) &&
    (
      editForm.city !== (selectedController?.affiliation.city || '') ||
      editForm.district !== (selectedController?.affiliation.district || '') ||
      editForm.dong !== (selectedController?.affiliation.dong || '')
    );
  const editEmailValidationMessage =
    !selectedController || (!hasEditEmailChanged && normalizedEditEmail.length > 0)
      ? ''
  ...
  const editPhoneValidationMessage =
    !selectedController || (!hasEditPhoneChanged && normalizedEditPhone.length > 0)
      ? ''
  ...
  const editRegionValidationMessage =
    !selectedController || (!hasEditRegionChanged && isEditRegionValid)
      ? ''
      : !isEditRegionValid
        ? '관리구역을 모두 선택해주세요.'
        : '관리구역이 올바릅니다.';
  ...
  /**
   * 검색어를 기준으로 관제요원 리스트를 이름/이메일/연락처/소속으로 필터링합니다.
   */
  const filteredControllers = useMemo(() => {
    const keyword = searchTerm.trim().toLowerCase();
    if (!keyword) return controllers;

    return controllers.filter((entry) =>
  ...
  /**
   * 관제요원 수정 패널의 관리구역 선택값을 한 번에 갱신합니다.
   */
  ...
  /**
   * 관제요원 등록 모달의 관리구역 선택값을 한 번에 갱신합니다.
   */
  ...
  /**
   * 좌측 리스트에서 선택한 관제요원 계정을 우측 상세 패널에 반영합니다.
   */
  const handleRowClick = (staff: PendingStaffApproval) => {
    setSelectedControllerId(staff.id);
  };

  /**
   * 우측 상세 패널에서 관제요원 기본정보, 관리구역, 계정 상태를 한 번에 저장합니다.
   */
  const handleDetailSave = async () => {
    ...
    const hasProfileChanged =
      selectedController.email !== normalizedEditEmail ||
      normalizedSelectedControllerPhone !== normalizedEditPhone ||
      selectedController.role !== nextEditForm.role ||
      (selectedController.affiliation.city || '') !== nextEditForm.city ||
      (selectedController.affiliation.district || '') !== nextEditForm.district ||
      (selectedController.affiliation.dong || '') !== nextEditForm.dong;
    const hasStatusChanged = selectedController.accountStatus !== accountStatusDraft;
    ...
    try {
      setProcessingId(selectedController.id);

      if (hasProfileChanged) {
        const profileOk = await adminService.updateStaffAccount(selectedController.id, nextEditForm);
        if (!profileOk) {
          alert('관제요원 정보 수정에 실패했습니다.');
          return;
        }
      }

      if (hasStatusChanged) {
        const statusOk = await adminService.updateStaffApproval(selectedController.id, accountStatusDraft);
        if (!statusOk) {
          alert('관제요원 계정 상태 변경에 실패했습니다.');
          return;
        }
      }
  ...
  const isProfileChanged = selectedController
    ? selectedController.email !== normalizedEditEmail ||
      normalizedSelectedControllerPhone !== normalizedEditPhone ||
      selectedController.role !== editForm.role ||
      (selectedController.affiliation.city || '') !== editForm.city ||
      (selectedController.affiliation.district || '') !== editForm.district ||
      (selectedController.affiliation.dong || '') !== editForm.dong
    : false;
  const isAccountStatusChanged = selectedController
    ? selectedController.accountStatus !== accountStatusDraft
    : false;
  const isDetailChanged = isProfileChanged || isAccountStatusChanged;

  /**
   * 관제요원을 어드민에서 직접 등록합니다.
   */
  const handleCreateController = async () => {
    if (!createForm.name || !createForm.email || !createForm.password || !createForm.phone || !createForm.city || !createForm.district || !createForm.dong) {
      alert('이름, 연락처, 이메일, 비밀번호, 시/도, 시/군/구, 읍/면/동은 필수입니다.');
      return;
    }
    ...
    try {
      ...
      const ok = await adminService.createStaff('controller', {
        ...createForm,
        phoneVerificationToken: createVerificationToken,
      });
      if (!ok) {
        alert('관제요원 등록에 실패했습니다.');
        return;
      }
  ...
  const handleDeleteController = async () => {
    if (!selectedController) return;
    const confirmed = window.confirm('삭제하시겠습니까?\n삭제된 관제요원 계정은 복구할 수 없습니다.');
    if (!confirmed) return;

    try {
      setDeletingStaffId(selectedController.id);
      const ok = await adminService.deleteStaffAccount(selectedController.id);
      if (!ok) {
        alert('관제요원 삭제에 실패했습니다.');
        return;
      }
  ...
  return (
    <div className="flex h-full min-h-0 bg-slate-100 overflow-hidden">
      <div className={`flex-1 min-w-0 flex flex-col p-5 transition-all duration-300 ${selectedController ? 'w-1/2' : 'w-full'}`}>
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-[28px] leading-none text-black">관제요원관리</h2>
            <p className="mt-1 text-[14px] text-black">관제센터 운영 계정과 담당 관리구역을 관리합니다.</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-300 overflow-hidden flex-1 flex flex-col shadow-md">
            ...
            <table className="w-full table-fixed border-collapse text-left">
              <thead className="sticky top-0 bg-slate-100 z-10">
                <tr className="border-b border-slate-300">
                  ...
                  <th className="w-[25%] px-2.5 py-3 text-[12px] text-black uppercase whitespace-nowrap">관리구역</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {loading ? (
                  ...
                  filteredControllers.map((staff) => (
                    <tr
                      key={staff.id}
                      onClick={() => handleRowClick(staff)}
                      className={`cursor-pointer transition-colors group ${selectedController?.id === staff.id ? 'bg-blue-100' : 'hover:bg-slate-100'}`}
                    >
                      <td className="px-2.5 py-2.5">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm text-black ${selectedController?.id === staff.id ? 'bg-blue-200' : 'bg-slate-100'}`}>
                            {staff.name[0]}
                          </div>
          ...
          <div className="border-t border-slate-300 bg-slate-100 px-4 py-2 text-[13px] text-slate-600">
            총 <span className="text-black">{filteredControllers.length}</span>명
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-5 pb-8">
            <div className="mb-5 rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm">
                ...
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="truncate text-[22px] text-black">{selectedController.name}</h3>
                      {renderAccountStatusBadge(selectedController.accountStatus, 'detail')}
                    </div>
                    <p className="mt-1 text-[13px] text-slate-500">관제센터 운영 계정</p>
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <button
                    onClick={handleDeleteController}
                    disabled={deletingStaffId === selectedController.id || processingId !== null}
                  ...
                  <button
                    ...
                    className="inline-flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 text-[14px] text-white hover:bg-blue-700 disabled:opacity-50"
                  >
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                  ...
                  <p className="mt-1 break-all text-[14px] text-black">{selectedController.email || '이메일 미입력'}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                  <p className="text-[12px] text-slate-500">관리구역</p>
                  <p className="mt-1 break-words text-[14px] text-black">
                    {[selectedController.affiliation.city, selectedController.affiliation.district, selectedController.affiliation.dong]
                      .filter(Boolean)
                      .join(' / ') || '소속 미입력'}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                  ...
                  <p className="mt-1 text-[14px] text-black">{formatPhoneNumber(selectedController.phone || '') || '연락처 미입력'}</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
                  <p className="text-[12px] text-slate-500">등록일시</p>
                  <p className="mt-1 text-[14px] text-black">{formatSubmittedAt(selectedController.createdAt)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="rounded-lg border border-slate-200 bg-white p-2.5">
                    <label className="block">
                      ...
                      <div className="mt-2 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-[13px] text-blue-700">
                        {editForm.email || '이메일을 입력해주세요.'}
                      </div>
                      <div className={controllerEmailDomainGridClass}>
                        <input
                          value={editEmailId}
                          onChange={(e) => syncEditEmail(e.target.value, editEmailDomain, editEmailCustomDomain)}
                          placeholder="이메일 아이디"
                          className="w-full rounded-lg border border-slate-200 px-3 py-2 text-[14px] text-black outline-none focus:border-blue-500"
                        />
                        ...
                        <select
                          ...
                          className={controllerFormSelectClass}
                        >
                      ...
                      {editEmailDomain === EMAIL_DOMAIN_DIRECT_VALUE && (
                        <input
                          ...
                          className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-[14px] text-black outline-none focus:border-blue-500"
                        />
                      )}
                    ...
                    <label className="block">
                      ...
                      <div className="mt-2 grid grid-cols-[96px_1fr] gap-2">
                        <select
                          ...
                          className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[14px] text-black outline-none focus:border-blue-500"
                        >
              ...
              <div className="rounded-xl border border-slate-300 bg-slate-100 p-4">
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-2.5">
                  <p className="text-[13px] text-black flex items-center gap-2 mb-2">
              ...
              <div className="rounded-xl border border-slate-300 bg-slate-100 p-4">
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-[140px_1fr]">
                  <div className="rounded-lg border border-slate-200 bg-white px-3 py-2.5">
                    ...
                    <div>{renderAccountStatusBadge(selectedController.accountStatus, 'detail')}</div>
                  </div>
                  <div className="rounded-lg border border-slate-200 bg-white p-2.5">
                    <label className="block">
              ...
              {selectedController.accountStatus === 'pending' && (
                <div className="rounded-xl border border-blue-200 bg-white p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => handleApproval(selectedController.id, 'active')}
                    ...
                    <button
                      onClick={() => handleApproval(selectedController.id, 'rejected')}
                      disabled={processingId !== null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 sm:p-6">
          ...
          <div className="relative flex max-h-[calc(100vh-32px)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-4">
              <div>
                <h3 className="text-[22px] text-black">관제요원 등록</h3>
              </div>
              <button
                ...
                <div className="block">
                  ...
                  <div className="mt-1 grid grid-cols-[96px_1fr_auto] gap-2">
                    <select
                      ...
                      className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-[15px] text-black outline-none focus:border-blue-500"
                    >
                      <option value="010">010</option>
                      <option value="011">011</option>
                      <option value="016">016</option>
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      placeholder="0000-0000"
                      className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-[15px] text-black outline-none focus:border-blue-500"
                    />
                    <button
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_104px]">
                  <input
                    ...
                    className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-[14px] text-black outline-none focus:border-blue-500"
                  />
                  <button
                    onClick={handleVerifyPhoneCode}
                    disabled={verificationChecking || !isPhoneVerificationRequested || !createVerificationCode}
                    className="inline-flex h-10 items-center justify-center rounded-lg bg-blue-600 px-3.5 text-[14px] text-white hover:bg-blue-700 disabled:opacity-50"
                  >
                    ...
                    <div className="block">
                      ...
                      {createEmailDomain === EMAIL_DOMAIN_DIRECT_VALUE && (
                        <input
                          ...
                          placeholder="메일주소 직접 입력"
                          className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-[15px] text-black outline-none focus:border-blue-500"
                        />
                      )}
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4 bg-slate-50">
              ...
              <button onClick={handleCreateController} disabled={createProcessing || !isCreateControllerFormValid} className="px-4 py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">
                등록
              </button>
            </div>
          </div>
