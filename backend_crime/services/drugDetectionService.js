/**
 * 마약 탐지 서비스
 * STARMAX 생체데이터 기반 마약/약물 사용 상태 실시간 분석
...
class DrugDetectionService {
  ...
  createDrugTransformerModel() {
    ...
    const maxPooled = tf.layers
      .globalMaxPooling1d({ name: "drug_max_pooling" })
      .apply(attention);
    const avgPooled = tf.layers
      .globalAveragePooling1d({ name: "drug_avg_pooling" })
      .apply(attention);
    const pooled = tf.layers
      .concatenate({ name: "drug_combined_pooling" })
      .apply([maxPooled, avgPooled]);

    // Multi-class classification head (4 classes: normal, stimulant, depressant, hallucinogen)
    const dense1 = tf.layers
      .dense({ units: 128, activation: "relu", name: "drug_classifier_1" })
      .apply(pooled);
    const dropout1 = tf.layers
      .dropout({ rate: 0.4, name: "drug_dropout_1" })
      .apply(dense1);
    const dense2 = tf.layers
      .dense({ units: 64, activation: "relu", name: "drug_classifier_2" })
      .apply(dropout1);
    const dropout2 = tf.layers
      .dropout({ rate: 0.3, name: "drug_dropout_2" })
      .apply(dense2);
    const output = tf.layers
      .dense({ units: 4, activation: "softmax", name: "drug_prediction" })
      .apply(dropout2);
  ...
  async checkDrugLLM() {
    try {
      ...
      const hasDrugModel = models.some((m) =>
        m.name.includes("goldentime-drug"),
      );
  ...
  async detectDrug(biometricData, userBaseline) {
    try {
      ...
      // 1단계: 불규칙 패턴 분석
      const irregularityAnalysis = await this.analyzeIrregularPatterns(
        biometricData,
        userBaseline,
      );
      ...
      const transformerPrediction =
        await this.runDrugTransformerAnalysis(biometricData);

      // 3단계: 시계열 변동성 분석
      const volatilityAnalysis =
        await this.analyzeVolatilityPatterns(biometricData);

      // 4단계: LLM 기반 종합 해석
      const llmAnalysis = await this.runDrugLLMAnalysis(
        biometricData,
        userBaseline,
      ...
      const requires_emergency_response =
        finalResult.drug_severity === "danger" ||
        finalResult.drug_severity === "critical";
      ...
      return {
        ...
        explanation: this.generateDrugExplanation(
          finalResult,
          biometricData,
          userBaseline,
        ),
        ...
        detailed_analysis: {
          irregularity_patterns: irregularityAnalysis,
          transformer_classification: transformerPrediction,
          volatility_metrics: volatilityAnalysis,
  ...
  async analyzeIrregularPatterns(biometricData, userBaseline) {
    ...
    // 3. HRV 급격한 변화 분석
    patterns.hrv_disruption = this.analyzeHRVDisruption(
      recentSequence,
      userBaseline,
    );

    // 4. 체온 변동 패턴
    patterns.temp_irregularity = this.analyzeTempIrregularity(
      recentSequence,
      userBaseline,
    );

    // 5. 움직임 패턴 교란
    patterns.movement_disruption =
      this.analyzeMovementDisruption(recentSequence);

    // 6. 호흡 불규칙성 (가능한 경우)
    patterns.respiratory_irregularity =
      this.analyzeRespiratoryIrregularity(recentSequence);
    ...
    const overall_score =
      irregularityScores.length > 0
        ? irregularityScores.reduce((sum, score) => sum + score, 0) /
          irregularityScores.length
        : 0;
  ...
  detectHeartRateSpikes(sequence) {
    ...
    for (let i = window; i < hrValues.length - window; i++) {
      const before =
        hrValues.slice(i - window, i).reduce((sum, v) => sum + v, 0) / window;
      const current = hrValues[i];
      const after =
        hrValues.slice(i + 1, i + window + 1).reduce((sum, v) => sum + v, 0) /
        window;
      ...
      if (current > before * 1.2 && current > after * 1.2) {
        spikes.push({
          timestamp: i,
          magnitude:
            (current - Math.max(before, after)) / Math.max(before, after),
    ...
    const severity = Math.min(
      spikes.length * 0.2 +
        spikes.reduce((sum, spike) => sum + spike.magnitude, 0) /
          Math.max(spikes.length, 1),
      1.0,
    );
  ...
  analyzeHRVDisruption(sequence, userBaseline) {
    const hrvValues = sequence
      .map((d) => d.hrv || d.stressLevel)
      .filter((hrv) => hrv > 0);
  ...
  analyzeMovementDisruption(sequence) {
    ...
    const accelerations = movementData.map((d) =>
      Math.sqrt(
        d.acceleration.x ** 2 + d.acceleration.y ** 2 + d.acceleration.z ** 2,
      ),
    );
  ...
  analyzeRespiratoryIrregularity(sequence) {
    const respiratoryData = sequence
    ...
    // 호흡수 변동성 계산
    const mean =
      respiratoryData.reduce((sum, r) => sum + r, 0) / respiratoryData.length;
...
위 생체데이터와 의료 정보를 종합하여 마약 사용 가능성 및 의학적 심각도를 5단계로 판정해주세요.

판정 기준:
1. 정상 (Normal): 특이사항 없음
2. 주의 (Caution): 경미한 생체신호 변화, 초기 반응 의심
3. 경고 (Warning): 뚜렷한 불규칙 패턴, 기능 저하 예상
4. 위험 (Danger): 심각한 생체신호 이탈, 중독 징후 뚜렷
5. 응급 (Critical): 생명 위협, 과다복용(Overdose) 의심, 즉각 조치 필요

응답 형식: "마약사용: [없음/각성제/억제제/환각제] | 심각도: [정상/주의/경고/위험/응급] | 근거: [상세분석] | 신뢰도: [0.0-1.0]"
    `;
  ...
  integrateDrugPredictions({ irregularity, transformer, volatility, llm }) {
    ...
    const llmScore =
      llm.severity === "critical"
        ...
        : llm.severity === "danger"
          ? 0.9
          : llm.severity === "warning"
            ? 0.7
            : llm.severity === "caution"
              ? 0.4
              : 0.1;
    ...
    // 최종 판정 (의학적 5단계 기준)
    const drug_detected = combinedScore > 0.3; // 주의 단계 이상이면 감지로 판단

    // 마약 유형 결정 (Transformer와 LLM 결과 종합)
    let suspected_drug_type = "none";
    if (drug_detected) {
      if (
        transformer.drug_type !== "normal" &&
        transformer.drug_type !== "unknown"
      ) {
    ...
    // 심각도 결정 (5단계)
    let drug_severity = "normal";
    if (combinedScore > 0.85) {
      drug_severity = "critical";
    } else if (combinedScore > 0.7) {
      drug_severity = "danger";
    } else if (combinedScore > 0.55) {
      drug_severity = "warning";
    } else if (combinedScore > 0.3) {
      drug_severity = "caution";
    }
  ...
  predictDrugType(patterns) {
    ...
    if (patterns.movement_disruption?.change_frequency > 0.4)
      scores.stimulant += 0.2;

    // 억제제 패턴 (낮은 활동성)
    if (patterns.hr_volatility?.volatility < 0.1) scores.depressant += 0.2;
    if (patterns.movement_disruption?.change_frequency < 0.2)
      scores.depressant += 0.3;

    // 환각제 패턴 (불규칙성)
    if (patterns.hr_volatility?.volatility > 0.2) scores.hallucinogen += 0.3;
    if (patterns.respiratory_irregularity?.irregular)
      scores.hallucinogen += 0.2;
  ...
  generateDrugExplanation(result, biometricData, userBaseline) {
    ...
    const typeExplanation =
      result.suspected_drug_type === "stimulant"
        ? "각성제 사용 패턴"
        : result.suspected_drug_type === "depressant"
          ? "억제제 사용 패턴"
          : result.suspected_drug_type === "hallucinogen"
            ? "환각제 사용 패턴"
            : "마약 사용 패턴";
  ...
  generateDrugRecommendation(result) {
    ...
    switch (result.drug_severity) {
      ...
      case "danger":
        recommendations.push("🚨 [위험] 보호자 호출 및 격리 조치");
        recommendations.push("🔪 자해/타해 위험, 안전한 공간 확보");
        recommendations.push("🏥 중독 관리 센터 또는 전문 병원 긴급 방문");
        break;

      case "warning":
        recommendations.push("⚠️ [경고] 약물 사용 중단 및 전문가 상담");
        recommendations.push("🔍 정밀 독성학 검사 필요");
        recommendations.push("📞 중독 상담 핫라인 연결 권장");
        break;

      case "caution":
        recommendations.push("⚠️ [주의] 초기 약물 반응 의심");
        recommendations.push("👀 행동 변화 및 생체신호 지속 관찰");
        recommendations.push("💬 스트레스 관리 및 심리 상담 권장");
        break;
