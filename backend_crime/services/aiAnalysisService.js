  ...
  async generateMemberReport(memberId) {
    try {
      ...
      const prompt = `
        ...
        **분석 요청 사항:**
        1. **현재 상태 평가**: 생체 신호와 이력을 바탕으로 현재 건강 상태를 '안정', '주의', '위험' 중 하나로 판단하고 그 이유를 설명하세요.
        2. **주요 위험 요인**: 감지된 이상 징후나 주의해야 할 패턴이 있다면 지적하세요. (예: 불규칙한 심박, 잦은 낙상 등)
        3. **관리 조치 제안**: 관리자나 보호자가 취해야 할 구체적인 행동 3가지를 제안하세요.
  ...
  async generateIncidentReport(incidentData) {
    try {
      const prompt = `
        ...
        **지시 사항:**
        1. 사고 유형과 데이터를 바탕으로 잠재적인 위험성을 요약하세요.
