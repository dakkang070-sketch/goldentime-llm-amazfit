...
class SchoolViolenceDetectionService {
  ...
  async analyzeSituation(data) {
    try {
      ...
      // Expanded keywords for better coverage of casual/slang variations
      const extortionKeywords = ["돈", "만원", "천원", "가져와", "내놔", "뺏", "빌려줘", "상납", "지갑", "호주머니", "현금", "계좌", "금액"];
      const demandKeywords = ["가져와", "내놔", "내나", "내노", "줘", "달라", "있냐", "있어", "빨리", "뒤져", "나오면"];
      const threatKeywords = ["옥상", "따라와", "죽여", "뒤져", "가만 안", "튀어와", "남아", "끝나고", "맞을"];
      
      const foundExtortion = extortionKeywords.filter(kw => transcript.includes(kw));
      const foundDemand = demandKeywords.filter(kw => transcript.includes(kw));
      const foundThreat = threatKeywords.filter(kw => transcript.includes(kw));
      
      // Logic: Money related word + Demand word = Extortion
      // e.g. "돈" + "줘" or "지갑" + "있냐"
      const isSeriousExtortion = foundExtortion.length > 0 && foundDemand.length > 0;
      
      // Logic: Threat word present
      const isSeriousThreat = foundThreat.length > 0;
      ...
      if (isSeriousExtortion && analysis.category !== "금품 갈취") {
        ...
        const warningMsg = `[시스템 긴급 진단]: 금품 관련 키워드('${foundExtortion.join("', '")}')와 요구/강압적 표현('${foundDemand.join("', '")}')이 결합되어 금품 갈취 정황이 감지되었습니다.`;
        analysis.reasoning = analysis.reasoning ? `${analysis.reasoning}\n\n${warningMsg}` : warningMsg;
        analysis.keywords = [...new Set([...(analysis.keywords || []), ...foundExtortion, ...foundDemand])];
        ...
        analysis.severity = "Critical";
        // Append warning instead of replacing, and remove "(기존 분석 보완)"
        const warningMsg = `[시스템 긴급 진단]: 신체적 위협 또는 강압적 장소 이동 키워드('${foundThreat.join("', '")}')가 검출되었습니다. 2차 피해 예방을 위해 즉시 확인이 필요합니다.`;
        analysis.reasoning = analysis.reasoning ? `${analysis.reasoning}\n\n${warningMsg}` : warningMsg;
  ...
  async _callLLMOrHeuristic(transcript, biometrics) {
    ...
    // 3. Fallback to heuristic (Enhanced)
    const severeViolenceKeywords = [
      "때려", "죽여", "돈내놔", "싸움", "피나", "맞을래", "옥상", "묶어", "동영상", "칼", "상납", "뒤진다", "패버려"
    ];
    
    const emergencyKeywords = [
      "살려주세요", "살려줘", "구해줘", "도와줘", "112", "신고해", "갇혀", "감금", "납치", "못나가", "안보내", "가둬"
    ];
    
    const insultKeywords = [
      "새끼", "병신", "씨발", "시발", "지랄", "닥쳐", "미친", "년", "놈", "찐따", "호구", "장애인"
    ];

    const coercionKeywords = [
      "와봐", "따라와", "남아", "일로와", "일로", "내놔", "내나", "빌려줘", "가져와", "갚을게"
    ];

    const extortionKeywords = [
      "돈", "만원", "천원", "지갑", "현금", "계좌", "송금", "호주머니", "얼마", "상납", "뺏"
    ];

    const demandKeywords = [
      "내놔", "내나", "줘", "가져와", "빌려", "뺏", "입금", "보내", "털어", "뒤져서", "나오면"
    ];
    ...
    // Keyword matching
    const foundSevere = severeViolenceKeywords.filter((k) => transcript.includes(k));
    const foundEmergency = emergencyKeywords.filter((k) => transcript.includes(k));
    const foundInsult = insultKeywords.filter((k) => transcript.includes(k));
    const foundCoercion = coercionKeywords.filter((k) => transcript.includes(k));
    const foundExtortion = extortionKeywords.filter((k) => transcript.includes(k));
    const foundDemand = demandKeywords.filter((k) => transcript.includes(k));
    const foundPrank = prankKeywords.filter((k) => transcript.includes(k));
    
    const hasSevere = foundSevere.length > 0;
    const hasEmergency = foundEmergency.length > 0;
    const hasInsult = foundInsult.length > 0;
    const hasCoercion = foundCoercion.length > 0;
    const hasExtortion = foundExtortion.length > 0;
    const hasDemand = foundDemand.length > 0;
    const hasPrank = foundPrank.length > 0;

    // Logic Tree
    if (hasEmergency) {
        category = "긴급 구조 요청";
        severity = "Critical";
        reasoning = `[상황 분석]: 구조 요청 또는 감금/납치를 암시하는 긴급 키워드('${foundEmergency.join("', '")}')가 감지되었습니다.\n[심리 분석]: 발화자의 극도의 공포와 절박함이 느껴집니다.\n[위험 요소]: 즉각적인 신변 보호와 경찰 신고가 필요한 위급 상황입니다.`;
        score = 100;
    } else if (hasSevere) {
        if (hasPrank) {
            category = "장난 (과격한 표현)";
            severity = "Caution";
            reasoning = `[상황 분석]: 과격한 표현('${foundSevere.join("', '")}')이 사용되었으나, 장난 키워드('${foundPrank.join("', '")}')가 함께 감지되었습니다.\n[심리 분석]: 친구 간의 거친 농담일 가능성이 높습니다.\n[위험 요소]: 습관적인 언어 폭력 가능성 존재.`;
            score = 50;
        } else {
            category = "신체 폭력 및 위협";
            severity = "Critical";
            reasoning = `[상황 분석]: 신체적 위협이나 폭력을 암시하는 결정적 단어('${foundSevere.join("', '")}')가 감지되었습니다.\n[심리 분석]: 상대방을 제압하거나 공포심을 유발하려는 의도가 뚜렷합니다.\n[위험 요소]: 즉각적인 신체 피해 발생 가능성이 매우 높습니다.`;
            score = 95;
        }
    } else if (hasExtortion && hasDemand) {
        // Case: "돈(Extortion) 좀 줘(Demand)" or "호주머니(Extortion) 털어(Demand)"
        if (hasPrank) {
            category = "장난";
            severity = "Caution";
            reasoning = `[상황 분석]: 금전적인 언급이 있으나 장난스러운 분위기로 감지됩니다.\n[심리 분석]: 친구 간의 가벼운 대화일 수 있으나 주의가 필요합니다.`;
            score = 40;
        } else {
            category = "금품 갈취";
            severity = "Critical"; // Extortion is usually critical
            reasoning = `[상황 분석]: 금품('${foundExtortion.join("', '")}')을 요구하거나 갈취하려는 표현('${foundDemand.join("', '")}')이 명확히 감지되었습니다.\n[심리 분석]: 경제적 이득을 목적으로 한 계획적인 괴롭힘일 수 있습니다.\n[위험 요소]: 지속적인 금품 갈취 및 2차 피해 우려가 큽니다.`;
            score = 90;
        }
    } else if (hasInsult && hasCoercion) {
        // Case: "이 새끼야(Insult), 너 일로 와봐(Coercion)"
        if (hasPrank) {
            category = "장난";
            severity = "Caution";
            reasoning = `[상황 분석]: 욕설과 지시적 표현이 섞여 있으나, 장난스러운 맥락으로 추정됩니다.\n[심리 분석]: 친밀한 사이에서의 거친 대화일 수 있습니다.`;
            score = 45;
        } else {
            category = "언어 폭력 및 강요";
            severity = "Warning"; // Can be Critical depending on context, Warning for now
            reasoning = `[상황 분석]: 욕설('${foundInsult.join("', '")}')과 함께 상대방의 행동을 강제하는 표현('${foundCoercion.join("', '")}')이 감지되었습니다.\n[심리 분석]: 우월적 지위를 이용한 위협이나 공포 분위기 조성 시도가 보입니다.\n[위험 요소]: 정서적 학대 및 잠재적 신체 폭력으로 이어질 수 있습니다.`;
            score = 85;
        }
    } else if (hasInsult) {
        if (hasPrank) {
            category = "장난";
            severity = "Normal";
            reasoning = `[상황 분석]: 비속어가 사용되었으나 친구 간의 일상적인 장난으로 보입니다.`;
            score = 20;
        } else {
            category = "언어 폭력";
            severity = "Caution";
            reasoning = `[상황 분석]: 상대방을 비하하거나 모욕하는 욕설('${foundInsult.join("', '")}')이 감지되었습니다.\n[심리 분석]: 부정적인 감정 표출이나 공격성이 내재되어 있습니다.\n[위험 요소]: 지속될 경우 정서적 피해가 우려됩니다.`;
            score = 60;
        }
    } else if (hasCoercion) {
        category = "강요 의심";
        severity = "Caution";
        reasoning = `[상황 분석]: 상대방에게 특정 행동을 요구하거나 지시하는 표현('${foundCoercion.join("', '")}')이 감지되었습니다.\n[심리 분석]: 위계 관계에 의한 압박일 수 있습니다.`;
        score = 50;
    } else if (hasPrank) {
      category = "장난";
      severity = "Normal";
      reasoning = `[상황 분석]: 친구 간의 일상적인 대화나 가벼운 농담('${foundPrank.join("', '")}')이 감지되었습니다.\n[심리 분석]: 긍정적이고 우호적인 관계가 형성되어 있습니다.\n[위험 요소]: 학교 폭력 위험 징후는 발견되지 않았습니다.`;
      score = 10;
    } else {
    ...
    return {
      ...
      confidence: score,
      reasoning,
      keywords: [...foundSevere, ...foundInsult, ...foundCoercion],
      primaryEmotion: (hasSevere || (hasInsult && hasCoercion)) ? "위협/공포" : (hasInsult ? "공격성" : "평이함")
    };
  }
  ...
  async _callLocalInferenceServer(transcript) {
    try {
      ...
      if (response.data && response.data.analysis) {
        ...
        if (typeof analysisData === 'string') {
            try {
                ...
                logger.warn("[SchoolViolence] Failed to parse analysis JSON from server", e);
                return null;
            }
        }

        if (analysisData && analysisData.category) {
            logger.info("[SchoolViolence] Local Inference Success:", analysisData);
            return {
              category: analysisData.category,
              severity: analysisData.severity || "Warning",
              reasoning: analysisData.reasoning || "분석 결과 없음",
              primaryEmotion: analysisData.primaryEmotion || "Unknown",
              tone: analysisData.tone || "Unknown",
              atmosphere: analysisData.atmosphere || "Unknown",
      ...
      // logger.debug("Local inference server unreachable or error");
    }
    return null;
  }
