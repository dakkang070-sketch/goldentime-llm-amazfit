...
SLANG = ["누칼협", "알빠노", "꼽주다", "뇌절", "억까", "킹받네", "어쩔티비", "틀딱", "잼민이", "한남", "김치녀", "맘충", "패드립"]
SCHOOL_TERMS = ["셔틀", "빵셔틀", "와이파이셔틀", "담당일진", "조건만남", "몸캠", "상납", "전따", "은따"]
THREATS = ["죽여버린다", "옥상으로 와라", "가만 안 둔다", "맞고 싶냐", "눈 깔아라", "돈 내놔", "안 가져오면 뒤진다"]
DEMANDS = ["빵 사와", "숙제 해놔", "망 봐", "돈 빌려줘", "가방 들어", "체육복 내놔", "데이터 켜"]
VICTIM_RESPONSES = ["하지마...", "왜 그래...", "돈 없어...", "미안해...", "살려줘...", "선생님한테 이를거야...", "싫어..."]
PRANK_INDICATORS = ["ㅋㅋㅋ", "장난이야", "농담인거 알지?", "표정 봐라", "쫄았냐?", "아 웃겨", "킹받네"]
...
TEMPLATES = {
    ...
    "Verbal Abuse": [
        ...
        ["Speaker A: 와 진짜 {slang} 하네. 니가 사람이냐?", "Speaker B: 그만 좀 해...", "Speaker A: {slang} 못 알아듣냐? ㅋㅋㅋ"],
        ["Speaker A: 야 {school_term} 주제에 말이 많아.", "Speaker B: 나도 사람이야...", "Speaker A: 시끄러워 {insult}아."]
    ],
    "Extortion": [
        ...
        ["Speaker A: 야 데이터 셔틀 안하고 뭐하냐?", "Speaker B: 데이터 다 썼어...", "Speaker A: 핫스팟 켜라고 {insult}아."]
    ],
    "Coercion": [
        ...
        ["Speaker A: 야 {school_term} 해. 폰 내놔.", "Speaker B: 안돼...", "Speaker A: 맞고 줄래 그냥 줄래?"]
    ],
    "Prank": [
        ...
        ["Speaker A: 야 너 {slang} 알아? ㅋㅋㅋ", "Speaker B: 그게 뭔데?", "Speaker A: 아 이것도 모르냐? {prank}."],
        ["Speaker A: 너 오늘 {school_term} 당첨이다 ㅋㅋㅋ", "Speaker B: 아 하지마 ㅋㅋㅋ", "Speaker A: 농담이야 임마 ㅋㅋㅋ"]
    ],
    "Normal": [
        ...
        ["Speaker A: 야 어제 그 유튜브 봤냐? 완전 {slang} ㅋㅋㅋ", "Speaker B: 인정 ㅋㅋㅋ 개웃기더라.", "Speaker A: 구독자 100만 가겠던데?"]
    ]
}
...
def build_transcript(category):
    ...
    for line in template:
        ...
        if "{slang}" in text:
            ...
            keywords.append(word)
        if "{school_term}" in text:
            word = random.choice(SCHOOL_TERMS)
            text = text.replace("{school_term}", word)
            keywords.append(word)
        if "{threat}" in text:
