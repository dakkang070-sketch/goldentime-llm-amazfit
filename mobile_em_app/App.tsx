...
const INITIAL_DEVICE_SETTINGS: DeviceSettings = {
  ...
  deviceId: 'SMX-UG1-8823',
  deviceName: 'STARMAX WATCH ULTRA G1',
          ...
          {stage === 2 && (
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                ...
                <p className="text-[10px] text-slate-400 mt-1">연결 실패 시 보호자에게 연락합니다</p>
              </div>
...
const SignupForm = memo(({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await backendService.signup({
        ...
        bloodType: formData.bloodType,
        emergencyContacts: (() => {
          const name = formData.emergencyName?.trim();
          const relationship = (formData.emergencyRelation || '보호자').trim();
      ...
      if (response.success) {
        ...
        // 구체적인 오류 메시지 표시
        if (response.message?.includes('이미 가입된')) {
          alert('이미 가입된 이메일 또는 전화번호입니다.');
        } else if (response.message?.includes('필수')) {
          alert(`필수 정보가 누락되었습니다: ${response.message}`);
        } else {
          alert(`회원가입에 실패했습니다: ${response.message || response.error || '서버 오류가 발생했습니다.'}`);
        }
  ...
  const inputBase = "w-full px-4 py-3 bg-white text-slate-900 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none placeholder:text-slate-400 font-normal transition-all shadow-sm";
  const labelBase = "text-[10px] font-medium text-slate-500 uppercase tracking-widest ml-1 mb-1 flex items-center gap-1";
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4">
         <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1.5 font-medium uppercase tracking-wider">
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="password" value={formData.password} onChange={e => handleChange('password', e.target.value)} placeholder="비밀번호 입력" className={inputBase} />
                  </div>
                </div>
              </div>
            </div>
...
const PersonalInfoEditor = memo(({ 
  ...
  const inputClass = "w-full px-4 py-3 bg-white text-slate-900 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none placeholder:text-slate-400 font-normal shadow-sm transition-all";
  const readOnlyInputClass = "w-full px-4 py-3 bg-slate-100 text-slate-500 border border-slate-200 rounded-xl outline-none font-normal shadow-sm select-none pointer-events-none";
  const requiredMark = <span className="text-red-500 font-normal">*</span>;
  ...
  return (
    <div className="px-4 pt-8 pb-24 animate-fade-in-up bg-slate-50">
      <div className="flex items-center justify-between mb-8 sticky top-0 bg-white/95 backdrop-blur z-20 py-2">
        ...
        <div className={`px-3 py-1 rounded-full text-[10px] font-normal border ${
      ...
      <div className="space-y-8">
        <section className="glass-panel p-5 rounded-2xl">
           ...
           <div className="space-y-4">
             <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1">
                <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">이름 {requiredMark}</label>
                <input type="text" value={formData.name} readOnly className={readOnlyInputClass} />
               </div>
               <div className="space-y-1">
                <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">생년월일 {requiredMark}</label>
                <input 
                  type="date" 
                  value={formData.dob} 
             ...
             <div className="space-y-1">
              <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">휴대폰 번호 {requiredMark}</label>
              <div className="flex gap-2">
                <input type="tel" value={formData.phone} readOnly className={readOnlyInputClass} />
                <button 
                  type="button" 
                  onClick={() => alert("본인인증(PASS) 서비스를 통해 변경할 수 있습니다.")}
                  className="px-4 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl text-sm font-normal shadow-sm hover:bg-slate-50 active:scale-95 transition-all whitespace-nowrap"
                >
                  변경
                </button>
              </div>
             ...
             <div className="grid grid-cols-3 gap-3">
               <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">나이</label>
               ...
               <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">성별</label>
               ...
               <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">혈액형</label>
             ...
             <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">신장 (cm)</label>
               ...
               <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">체중 (kg)</label>
         ...
         <section className="glass-panel p-5 rounded-2xl">
           ...
           <div className="space-y-4">
             <div className="space-y-1">
               <label className="text-[10px] font-normal text-slate-400 uppercase ml-1"><Pill size={10}/> 복용 중인 약물</label>
             ...
             <div className="space-y-1">
               <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">알레르기</label>
               <input type="text" value={formData.allergies} onChange={e => handleChange('allergies', e.target.value)} className={inputClass} />
             </div>
             <div className="space-y-1">
               <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">만성 질환</label>
         ...
         <section className="glass-panel p-5 rounded-2xl">
           ...
           <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">보호자 이름 {requiredMark}</label>
                  <input type="text" value={formData.emergencyName} onChange={e => handleChange('emergencyName', e.target.value)} className={inputClass} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">관계 {requiredMark}</label>
              ...
              <div className="space-y-1">
                 <label className="text-[10px] font-normal text-slate-400 uppercase ml-1">보호자 연락처 {requiredMark}</label>
         ...
         <section className="glass-panel p-5 rounded-2xl">
            ...
            <div className="space-y-5">
               ...
               <div className="space-y-1">
                 ...
                 <p className="text-[10px] text-slate-400 mt-3 pl-1 leading-relaxed">
    ...
    <div className="px-4 pt-8 pb-24 animate-fade-in-up bg-slate-50 min-h-screen">
      <div className="flex items-center justify-between mb-8 sticky top-0 bg-white/95 backdrop-blur z-20 py-2">
        ...
        <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 font-normal">연결됨</span>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <section className="glass-panel p-5 rounded-2xl relative overflow-hidden">
           ...
           <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <p className="text-[10px] text-slate-400 uppercase">Model</p>
                      <p className="text-sm text-slate-700 font-mono">{currentSettings.modelNumber || 'Unknown'}</p>
                  </div>
                  <div>
                      <p className="text-[10px] text-slate-400 uppercase">Manufacturer</p>
                  ...
                  <div>
                      <p className="text-[10px] text-slate-400 uppercase">Firmware</p>
           ...
           <div className="space-y-4">
              <div className="flex items-center justify-between py-2">
                 <div>
                    ...
                    <p className="text-[10px] text-slate-400">매일 아침 건강 리포트를 자동 생성합니다.</p>
                 </div>
           ...
           <div className="space-y-4 divide-y divide-slate-100">
              <div className="flex items-center justify-between py-2">
                 <div>
                    ...
                    <p className="text-[10px] text-slate-400">응급 상황 발생 시 위치 전송에 필요합니다.</p>
                 </div>
              ...
              <div className="flex items-center justify-between py-2 pt-4">
                 <div>
                    ...
                    <p className="text-[10px] text-slate-400">AI 알고리즘 학습 및 맞춤 분석에 활용됩니다.</p>
                 </div>
        ...
        <div className="pt-4 flex gap-3">
           ...
           <button onClick={() => onSave(currentSettings)} className="flex-[2] py-4 bg-slate-900 text-white font-normal rounded-2xl shadow-lg shadow-slate-200 active:scale-[0.98] transition-transform">
              설정 저장
           </button>
        </div>
      </div>
    ...
    <div className="px-4 pt-8 pb-24 animate-fade-in-up bg-slate-50 min-h-screen">
      ...
      <div className="space-y-6">
         <section className="glass-panel p-5 rounded-2xl">
           ...
           <div className="space-y-1">
             <p className="text-[10px] text-slate-400 uppercase font-normal">Company Name</p>
           ...
           <div className="mt-4 pt-4 border-t border-slate-100 space-y-1">
             <p className="text-[10px] text-slate-400 uppercase font-normal">Contact</p>
      ...
      <div className="mt-10 text-center">
         <p className="text-[10px] text-slate-300">Copyright © 2025 Prime Play Co., Ltd. All rights reserved.</p>
      </div>
    </div>
  );
});
...
export default function App() {
  ...
  const detectEmergency = (currentData: BiometricData): { isEmergency: boolean; severity: 'low' | 'medium' | 'high'; reason: string } => {
    ...
    // 심박수 체크
    if (currentData.heartRate < 40 || currentData.heartRate > 150) {
      reasons.push(currentData.heartRate < 40 ? '심박수가 너무 낮습니다 (브라디카르디아)' : '심박수가 너무 높습니다 (타키카르디아)');
      severity = 'high';
    } else if (currentData.heartRate > 120) {
    ...
    if (currentData.spO2 < 90) {
      ...
      severity = 'high';
    } else if (currentData.spO2 < 95) {
      reasons.push('산소포화도가 낮습니다');
      if (severity === 'low') severity = 'medium';
    }
    
    // 체온 체크
    if (currentData.temperature < 35 || currentData.temperature > 39) {
      reasons.push('체온이 비정상입니다');
      if (severity === 'low') severity = 'medium';
    }
    
    // 혈압 체크
    if (currentData.bloodPressureSys > 180 || currentData.bloodPressureDia > 110) {
      reasons.push('혈압이 위험합니다');
      severity = 'high';
    }
    
    // 호흡수 체크
    if (currentData.respiratoryRate < 8 || currentData.respiratoryRate > 30) {
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      ...
      const isDanger = reportText.includes('위험') || reportText.includes('응급') || emergencyCheck.isEmergency;
      
      const newReport: ReportItem = {
        id: Date.now().toString(),
        timestamp: Date.now(),
        content: emergencyCheck.isEmergency ? emergencyCheck.reason : reportText,
        status: isDanger ? 'danger' : reportText.includes('주의') || reportText.includes('높음') ? 'warning' : 'normal'
      };
      ...
      } else if (isDanger && emergencyStage === 0) {
        setEmergencyStage(1);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 백엔드 로그인 API 호출
      const response = await backendService.login(loginEmail, loginPassword);
      
      if (!response) {
        throw new Error('서버로부터 응답을 받을 수 없습니다.');
      }
      
      // API 응답 구조: { success: true, data: { user: {...}, token: "..." } }
      const responseData = (response as any).data || response; // data 속성이 있으면 사용, 없으면 response 자체 사용
      
      if (response.success && responseData.user) {
        // 토큰은 backendService.login 내부에서 이미 설정됨
        setUser({ 
        ...
        // 기기 연결 여부 확인 (정책: 기기 연결 시 바로 홈 화면, 아니면 기기 연결 화면)
        if (responseData.user.starmaxDevice) {
          setIsPaired(true);
          setDeviceSettings(prev => ({
             ...prev,
             deviceName: responseData.user.starmaxDevice.name || 'STARMAX Device',
             initialSyncComplete: true
          }));
        } else {
          setIsPaired(false);
        ...
        setActiveTab('home');
        console.log('로그인 성공:', responseData.user);
      } else {
  ...
  const startQrScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (jsQR) {
        console.log('jsQR 엔진 가동... (attemptBoth)');
        const code = jsQR(canvasImageData.data, width, height, {
        ...
        console.error('jsQR 엔진을 찾을 수 없습니다.');
      }
            ...
            setTimeout(async () => {
              try {
                ...
                if (devices.length > 0) {
                  ...
                  alert('QR 코드는 인식되었으나, 주변에서 해당 와치를 찾을 수 없습니다.\n\n1. 와치가 켜져 있는지\n2. 블루투스가 활성화되어 있는지\n3. 와치가 다른 기기와 연결되어 있지 않은지 확인해 주세요.');
                }
              } catch (err) {
  ...
  const connectDevice = async (device: StarmaxDevice) => {
    ...
    try {
      ...
      // STARMAX BLE 기기 연결
      const response = await starmaxService.connectDevice(device);
      ...
      if (response.success && response.data) {
        ...
        setDeviceSettings(prev => ({ 
          ...
          batteryLevel: connectedDevice.batteryLevel || 100,
          initialSyncComplete: true 
        }));
  ...
  const skipPairing = () => { setIsPaired(true); setActiveTab('home'); };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-8 bg-slate-50 transition-colors duration-500">
      ...
      <form onSubmit={handleLogin} className="w-full max-w-sm space-y-5 animate-fade-in-up">
        <div className="space-y-2">
          <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest ml-1">Email Address</label>
          <input 
            required 
            type="email" 
            value={loginEmail}
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest ml-1">Password</label>
          <input 
            required 
            type="password" 
            value={loginPassword}
        ...
        <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-2xl font-medium shadow-lg shadow-indigo-900/10 mt-4 transition-all active:scale-[0.98]">
          로그인하기
        </button>
      </form>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          {!isScanning && !isConnecting && !detectedQrMac && scanResults.length === 0 && (
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button 
                onClick={startScan} 
                className="px-8 py-3 bg-indigo-600 text-white rounded-full font-normal shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 active:scale-95 transition-transform"
              >
                <Bluetooth size={18} /> 일반 검색 시작
              </button>
              ...
              <div className="relative">
                <input 
                  ...
                  accept="image/*" 
                  capture="environment" 
                  onChange={startQrScan}
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                  id="qr-input"
                />
                <button className="w-full px-8 py-3 bg-white text-indigo-600 border-2 border-indigo-100 rounded-full font-normal flex items-center justify-center gap-2">
                  <Camera size={18} /> QR 코드로 찾기
                </button>
              </div>

              <button 
                onClick={() => handleManualMacInput('직접 주소를 입력하여 와치를 찾습니다.')}
                className="w-full px-8 py-3 bg-slate-100 text-slate-600 rounded-full font-normal flex items-center justify-center gap-2 text-sm"
              >
                <Search size={16} /> MAC 주소 직접 입력
              </button>
            </div>
          )}
            ...
            <div className="w-full space-y-3">
              ...
              {scanResults.map((device, i) => (
                <div 
                  ...
                  className="glass-panel p-4 rounded-2xl flex justify-between items-center cursor-pointer hover:bg-slate-50"
                >
                  <div className="flex items-center gap-3">
  ...
  const renderDashboardScreen = () => {
    ...
    return (
      <div className="px-0 pt-0 pb-20 bg-slate-50 min-h-screen">
        <header className="bg-white px-6 py-7 rounded-b-[40px] shadow-lg shadow-slate-900/10 mb-8 text-left">
          <div className="flex flex-col items-start">
            ...
            <div className="flex items-center gap-2 mt-3 text-xs text-slate-700">
              ...
              <span className="text-[11px] font-medium">실시간 데이터 모니터링 중</span>
            </div>
          </div>
          <div className="absolute top-6 right-6 z-20 flex items-center gap-3">
            <button onClick={triggerManualEmergency} className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-white/30 active:scale-95 transition-all">
        ...
        <div className="px-4">
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="col-span-1">
              {heartRateMetric && <MetricCard config={heartRateMetric} value={data.heartRate} status={data.heartRate > 150 ? 'danger' : data.heartRate > 100 ? 'warning' : 'normal'} className="h-full" />}
            </div>
            ...
            {otherMetrics.map((m) => (
               <MetricCard key={m.id} config={m} value={m.id === 'bloodPressure' ? `${data.bloodPressureSys}/${data.bloodPressureDia}` : (data as any)[m.id]} status="normal" className="h-full" />
            ))}
          </div>

          <div className="glass-panel p-5 rounded-2xl border-l-4 border-indigo-500 shadow-sm">
            <h3 className="font-normal text-sm flex items-center gap-2 text-slate-700 mb-2"><Sparkles size={14} className="text-indigo-600" />실시간 AI 코멘트</h3>
            <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap line-clamp-2 font-normal">{reportHistory[0]?.content || "데이터 분석 대기 중..."}</p>
          </div>
        </div>
      </div>
    );
  ...
  const renderProfileScreen = () => {
    ...
    if (profileView === 'device') return <DeviceManager settings={deviceSettings} onSave={handleDeviceSettingsSave} onCancel={() => setProfileView('main')} onDisconnect={() => { setIsPaired(false); setProfileView('main'); setScanResults([]); }} />;
      ...
      <div className="px-4 pt-12 pb-24 text-center bg-slate-50 min-h-screen">
        ...
        <div className="space-y-3 text-left">
          ...
          <div onClick={() => setProfileView('device')} className="glass-panel p-4 rounded-2xl flex items-center gap-4 cursor-pointer hover:bg-slate-50 transition-colors">
            ...
            <div className="flex-1"><p className="font-normal text-slate-700">연동 기기 관리</p><p className="text-[10px] text-indigo-500 font-normal uppercase">{deviceSettings.deviceName}</p></div><ChevronRight size={18} className="text-slate-300" />
          </div>
          ...
          <div onClick={() => setProfileView('about')} className="glass-panel p-4 rounded-2xl flex items-center gap-4 cursor-pointer hover:bg-slate-50 transition-colors">
            ...
            <div className="flex-1">
                ...
                <p className="text-[10px] text-slate-400 font-normal uppercase">Version 2.4.1</p>
            </div>
            <ChevronRight size={18} className="text-slate-300" />
          </div>
        </div>
           ...
           <div className="grid grid-cols-2 gap-3">
               <div className="glass-panel p-4 rounded-xl">
                  <div className="flex items-center gap-2 mb-2 text-slate-400">
                     <Signal size={14}/>
                     <span className="text-[10px] uppercase font-normal tracking-wider">신호 품질</span>
                  </div>
               ...
               <div className="glass-panel p-4 rounded-xl">
                  <div className="flex items-center gap-2 mb-2 text-slate-400">
                     <Battery size={14}/>
                     <span className="text-[10px] uppercase font-normal tracking-wider">배터리 효율</span>
                  </div>
  ...
  if (!user) { if (authMode === 'signup') return <SignupForm onSignup={handleSignupFull} onBack={() => setAuthMode('login')} />; return renderAuthScreen(); }
  if (!isPaired) return renderPairingScreen();
  ...
  return (
    <div className="min-h-screen bg-slate-50 max-w-lg mx-auto relative overflow-x-hidden">
