export interface RegionDistrictOption { name: string; areas: string[]; }
export interface RegionCityOption { name: string; districts: RegionDistrictOption[]; }

/**
 * 회원가입 지역소속 선택에 사용하는 전국 행정구역 카탈로그입니다.
 */
export const MEMBER_REGION_CATALOG: RegionCityOption[] = [{
        name: "강원특별자치도",
        districts: [{
            name: "강릉시",
...
/**
 * 선택한 시/도에 해당하는 시/군/구 목록을 반환합니다.
 */
export function getMemberDistrictOptions(city: string): RegionDistrictOption[] {
  const found = MEMBER_REGION_CATALOG.find((c) => c.name === city);
  return found ? found.districts : [];
}

/**
 * 선택한 시/도와 시/군/구에 해당하는 읍/면/동 목록을 반환합니다.
 */
export function getMemberAreaOptions(city: string, district: string): string[] {
  const dists = getMemberDistrictOptions(city);
  const found = dists.find((d) => d.name === district);
