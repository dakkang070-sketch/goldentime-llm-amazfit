...
router.post('/analyze', async (req, res) => {
  try {
    ...
    // 1단계: 사용자 베이스라인 조회
    const [alcoholBaseline, drugBaseline, psychoactiveBaseline] = await Promise.all([
    ...
    const userBaseline = {
      ...
      psychoactive: psychoactiveBaseline ? psychoactiveBaseline.getCNSBaselineStats() : null
    };
    ...
    const formattedResult = formatForEmergencySystem(analysisResult, biometricData);
    ...
    if (analysisResult.risk_assessment.level === 'critical') {
      emergencyTriggered = await triggerEmergencyResponse(userId, analysisResult, biometricData);
    }
...
router.post('/analyze/:type', async (req, res) => {
  try {
    ...
    // 개별 분석 실행
    const result = await substanceSystem.services[type].detect(biometricData, baseline);
...
router.post('/baseline/:userId', async (req, res) => {
  try {
    ...
    if (!substanceType || substanceType === 'all') {
      // 모든 물질 베이스라인 업데이트
      updateResults.alcohol = await updateAlcoholBaseline(userId, biometricData);
      updateResults.drug = await updateDrugBaseline(userId, biometricData);
      updateResults.psychoactive = await updatePsychoactiveBaseline(userId, biometricData);
    } else {
      // 특정 물질만 업데이트
      switch (substanceType) {
        ...
        case 'psychoactive':
          ...
          break;
        default:
          return res.status(400).json({ success: false, error: '지원하지 않는 물질 유형' });
      }
    }
...
router.post('/monitor/start/:userId', async (req, res) => {
  try {
    ...
    // 실시간 모니터링 시작
    const monitoringSession = await substanceSystem.startRealTimeMonitoring(userId, monitoringConfig);
    ...
    const status = {
      ...
      models_status: {
        ...
        psychoactive: {
          ...
          llm_available: await checkLLMAvailable('goldentime-psychoactive:latest'),
...
router.post('/retrain', async (req, res) => {
  try {
    ...
    // 재훈련 필요성 체크
    const retrainNeeded = forceRetrain || await checkRetrainingNeeded(substanceType);
  ...
  const formatted = {
    ...
    substance_analysis: {
      ...
      psychoactive: {
        ...
        drug_category: analysisResult.individual_analysis.psychoactive.drug_category,
        severity: analysisResult.individual_analysis.psychoactive.severity,
        cns_depression_level: analysisResult.individual_analysis.psychoactive.cns_depression_level,
        confidence: analysisResult.individual_analysis.psychoactive.confidence,
        explanation: analysisResult.individual_analysis.psychoactive.explanation
      }
    },
...
function determineHospitalPriority(analysisResult) {
  ...
  if (analysisResult.integrated_result.substance_detected) {
    ...
    if (analysisResult.integrated_result.detected_substances.includes('drug')) {
      priorities.push('독성학과');
    }
    
    if (analysisResult.integrated_result.detected_substances.includes('psychoactive')) {
...
function generateParamedicAlerts(analysisResult) {
  ...
  analysisResult.integrated_result.detected_substances.forEach(substance => {
    ...
    alerts.push({
      type: 'substance_detection',
      substance: substance,
      severity: analysis.severity,
      message: `${substance} 사용 의심 - ${analysis.severity} 수준`,
      confidence: analysis.confidence,
      recommendations: analysis.recommendation,
      priority: analysis.severity === 'severe' ? 'critical' : 
                analysis.severity === 'moderate' ? 'high' : 'medium'
    });
  });
  ...
  if (analysisResult.complex_interactions.length > 0) {
    alerts.push({
      ...
      message: '복합 물질 사용 감지 - 즉시 응급 대응 필요',
      interactions: analysisResult.complex_interactions,
      priority: 'critical'
    });
  }
...
async function triggerEmergencyResponse(userId, analysisResult, biometricData) {
  try {
    ...
    const emergencyCase = new EmergencyCase({
      ...
      emergencyLevel: analysisResult.risk_assessment.level === 'critical' ? 5 : 4,
      detectedAnomalies: analysisResult.integrated_result.detected_substances.map(substance => ({
        type: `${substance}_abuse`,
        description: analysisResult.individual_analysis[substance].explanation,
        severity: analysisResult.individual_analysis[substance].severity,
        confidence: analysisResult.individual_analysis[substance].confidence
      })),
      
      // 물질 분석 특화 정보
      substanceAnalysis: {
        detectedSubstances: analysisResult.integrated_result.detected_substances,
...
async function recordSubstanceEvents(userId, analysisResult, biometricData) {
  try {
    const detectedSubstances = analysisResult.integrated_result.detected_substances;
    ...
    logger.info(`✅ 물질 사용 이벤트 기록 완료: ${detectedSubstances.join(', ')}`);
  ...
  try {
    ...
    if (alcoholBaseline) {
      alcoholBaseline.alcohol_events
        .filter(event => event.detected_at >= options.startDate && event.detected_at <= options.endDate)
        .forEach(event => {
          history.push({
...
function calculateRiskLevels(history) {
  ...
  history.forEach(event => {
    const risk = event.severity === 'severe' ? 'critical' :
                  event.severity === 'moderate' ? 'high' :
                  event.severity === 'mild' ? 'medium' : 'low';
    riskLevels[risk]++;
  });
  return riskLevels;
}
...
async function checkLLMAvailable(modelName) {
  try {
    ...
    return response.data.models.some(m => m.name.includes(modelName.split(':')[0]));
  } catch {
    return false;
  }
}
...
function estimateTreatmentDuration(analysisResult) {
  ...
  const substanceCount = analysisResult.integrated_result.detected_substances.length;
