...
router.get('/info/:hpid', requireAuth, async (req, res) => {
  try {
    ...
    const result = await emergencyHospitalService.getEmergencyHospitalInfo(hpid);
...
router.get('/severe-disease/:hpid', requireAuth, async (req, res) => {
  try {
    ...
    const result = await emergencyHospitalService.getSevereDiseaseAvailability(hpid);
