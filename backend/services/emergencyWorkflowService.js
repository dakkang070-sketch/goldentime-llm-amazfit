...
const { emitEmergencyCaseCreated, emitStatusUpdate, emitEscalation } = require('./socketService');
const { notifyEmergencyToGuardian, notifyParamedic, notifyHospital } = require('./notificationService');
...
class EmergencyWorkflowService {
  ...
  async initiateEmergencyWorkflow(emergencyCaseId, options = {}) {
    try {
      ...
      // 타임라인 기록
      this.addTimelineEvent(emergencyCaseId, 'workflow_initiated', '응급 워크플로우 시작');
      ...
      await this.handleWorkflowFailure(emergencyCaseId, 'initiation_failed', error.message);
      throw error;
    }
  }
  ...
  async executeParamedicMatching(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'paramedic_matching_started', '응급구조사 매칭 시작');
    ...
    try {
      ...
      if (matchResult.matched) {
        ...
        this.addTimelineEvent(emergencyCaseId, 'paramedic_matched', `응급구조사 매칭 완료: ${matchResult.paramedicId}`);
  ...
  async monitorParamedicResponse(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'paramedic_enroute_monitoring', '현장 도착 모니터링 시작');
  ...
  async executeHospitalMatching(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'hospital_matching_started', '이송 병원 매칭 시작');
    ...
    try {
      ...
      // 환자 상태 기반 전문 병원 매칭
      const specialtyRequirements = this.determineSpecialtyRequirements(emergencyCase);
      
      const hospitalMatch = await autoMatchHospitalForCase(emergencyCaseId, {
        specialties: specialtyRequirements,
        prioritizeDistance: emergencyCase.emergencyLevel >= 4,
        requireICU: emergencyCase.emergencyLevel === 5
      });
      ...
      if (hospitalMatch.matched) {
        ...
        this.addTimelineEvent(emergencyCaseId, 'hospital_matched', `이송 병원 확정: ${hospitalMatch.hospitalId}`);
  ...
  async calculateTransportRoute(emergencyCaseId) {
    ...
    try {
      const emergencyCase = await EmergencyCase.findById(emergencyCaseId).lean();
      ...
      this.addTimelineEvent(emergencyCaseId, 'route_calculated', 
  ...
  async handleParamedicArrival(emergencyCaseId, paramedicId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'paramedic_arrived', '응급구조사 현장 도착');
  ...
  async handleTransportStart(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'transport_started', '병원으로 이송 시작');
  ...
  async completeWorkflow(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'workflow_completed', '응급 대응 완료');
  ...
  async escalateParamedicMatching(emergencyCaseId) {
    ...
    logger.warn(`응급구조사 매칭 에스컬레이션: ${emergencyCaseId} (Level ${workflow.escalationLevel})`);

    this.addTimelineEvent(emergencyCaseId, 'escalation_paramedic', `응급구조사 매칭 에스컬레이션 Level ${workflow.escalationLevel}`);
  ...
  async requestAirAmbulance(emergencyCaseId) {
    ...
    this.addTimelineEvent(emergencyCaseId, 'air_ambulance_requested', '헬기 응급실 요청');
  ...
  async monitorActiveWorkflows() {
    ...
    const escalatedCount = Array.from(this.activeWorkflows.values())
      .filter(w => w.escalationLevel > 0).length;
    ...
    for (const [caseId, workflow] of this.activeWorkflows) {
      ...
      if (elapsedMinutes > this.SLA_TIMES.TOTAL_RESPONSE && workflow.escalationLevel === 0) {
