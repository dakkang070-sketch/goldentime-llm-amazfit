...
const { collectBatchTrainingData, validateTrainingData } = require('./fineTuningService');
...
class AutoLearningService {
  ...
  start() {
    ...
    logger.info(`📊 실시간 학습 임계치: ${AUTO_LEARNING_CONFIG.MIN_NEW_DATA_THRESHOLD}개`);
  }
  ...
  async getNewDataCount() {
    const lastTrainingTime = this.lastTrainingTime || new Date(Date.now() - 24 * 60 * 60 * 1000);
  ...
  async executeBatchLearning() {
    ...
    try {
      ...
      if (trainingData.length < AUTO_LEARNING_CONFIG.MIN_NEW_DATA_THRESHOLD) {
        logger.info(`📊 데이터 부족: ${trainingData.length}개 (최소: ${AUTO_LEARNING_CONFIG.MIN_NEW_DATA_THRESHOLD}개)`);
        return;
      }
      ...
      if (performance.accuracy >= AUTO_LEARNING_CONFIG.MIN_ACCURACY_THRESHOLD) {
        if (AUTO_LEARNING_CONFIG.ENABLE_AUTO_DEPLOY) {
          ...
          logger.info(`✅ 배치 학습 완료 및 자동 배포: 정확도 ${performance.accuracy * 100}%`);
        } else {
          logger.info(`✅ 배치 학습 완료 (수동 배포 대기): 정확도 ${performance.accuracy * 100}%`);
        }
      } else {
        await this.rollbackModel();
        logger.warn(`❌ 성능 기준 미달 (${performance.accuracy * 100}% < ${AUTO_LEARNING_CONFIG.MIN_ACCURACY_THRESHOLD * 100}%). 롤백 완료.`);
      }
      ...
      this.modelPerformanceHistory.push({
        ...
        deployed: performance.accuracy >= AUTO_LEARNING_CONFIG.MIN_ACCURACY_THRESHOLD && AUTO_LEARNING_CONFIG.ENABLE_AUTO_DEPLOY
      });
    ...
    try {
      ...
      if (newData.length >= AUTO_LEARNING_CONFIG.MIN_NEW_DATA_THRESHOLD) {
        ...
        if (performance.accuracy >= AUTO_LEARNING_CONFIG.MIN_ACCURACY_THRESHOLD) {
          if (AUTO_LEARNING_CONFIG.ENABLE_AUTO_DEPLOY) {
            await this.deployNewModel(newModelName);
            logger.info(`⚡ 증분 학습 완료 및 배포: 정확도 ${performance.accuracy * 100}%`);
          }
        }
  ...
  async prepareTrainingData() {
    ...
    const data = await collectBatchTrainingData(AUTO_LEARNING_CONFIG.MAX_DATA_THRESHOLD);
    ...
    for (const item of data) {
      ...
      if (validation.valid) {
        ...
        logger.warn('데이터 품질 문제', { reason: validation.reason, input: item.input?.substring(0, 50) });
      }
    }
    
    logger.info(`📊 유효한 훈련 데이터: ${validData.length}개 (전체: ${data.length}개)`);
    
    // 업데이트된 데이터셋 저장
    const outputPath = path.join(process.cwd(), 'backend/data/fine-tuning-dataset-auto.json');
  ...
  async trainIncrementalModel(newData) {
    ...
    // 기존 데이터와 새 데이터 결합
    const existingDataPath = path.join(process.cwd(), 'backend/data/fine-tuning-dataset-combined.json');
    let existingData = [];
    ...
    // 업데이트된 데이터셋 저장
    fs.writeFileSync(existingDataPath, JSON.stringify(uniqueData, null, 2), 'utf8');
  ...
  async validateModelPerformance(modelName) {
    ...
    try {
      ...
      const testCases = [
        {
          input: "기초선 심박수: 70 bpm, 현재 심박수: 45 bpm, 스트레스: 85, 움직임: 정상, 위치: 서울시 강남구",
          expectedKeywords: ["심박수", "낮", "권고", "확인"]
        },
        {
          input: "기초선 심박수: 70 bpm, 현재 심박수: 160 bpm, 스트레스: 95, 움직임: 낙상 감지, 위치: 서울시 서초구",
      ...
      logger.info(`🧪 성능 검증 결과: ${accuracy * 100}% (${correctPredictions}/${testCases.length})`);
  ...
  async deployNewModel(newModelName) {
    ...
    try {
      ...
      await execAsync(`ollama cp goldentime-emergency:latest goldentime-emergency:previous`);
  ...
  async rollbackModel() {
    ...
    try {
      await execAsync(`ollama cp goldentime-emergency:previous goldentime-emergency:latest`);
...
PARAMETER num_ctx 4096`;
  ...
  analyzeDataDistribution(trainingData) {
    ...
    trainingData.forEach(item => {
      ...
      if (output.includes('즉시') || output.includes('위급') || output.includes('생명')) {
        critical++;
    ...
    return { critical, warning, normal: trainingData.length - critical - warning };
  }
  ...
  async getNewTrainingData() {
    const lastTrainingTime = this.lastTrainingTime || new Date(Date.now() - 60 * 60 * 1000);
    
    const newCases = await EmergencyCase.find({
      'llmAnalysis.analysisText': { $exists: true },
      emergencyLevel: { $gte: 3 },
      detectedAt: { $gte: lastTrainingTime }
    }).populate('userId', 'baselineBiometric').limit(100);
  ...
  async cleanupOldBackups() {
    try {
      const { stdout } = await execAsync('ollama list | grep "goldentime-emergency:backup-"');
      const backupModels = stdout.split('\n')
      ...
      if (backupModels.length > AUTO_LEARNING_CONFIG.MAX_MODEL_BACKUPS) {
        const modelsToDelete = backupModels.slice(AUTO_LEARNING_CONFIG.MAX_MODEL_BACKUPS);
  ...
  getTrainingStats() {
    return {
      ...
      averageAccuracy: this.modelPerformanceHistory.length > 0 
        ? this.modelPerformanceHistory.reduce((sum, h) => sum + h.accuracy, 0) / this.modelPerformanceHistory.length
        : 0,
