...
class AlcoholDetectionService {
  ...
  createTransformerModel() {
    ...
    const pooled = tf.layers.globalAveragePooling1d({ name: 'global_avg_pooling' }).apply(attention);
    
    // Classification Head
    const dense1 = tf.layers.dense({ units: 64, activation: 'relu', name: 'classifier_1' }).apply(pooled);
    const dropout = tf.layers.dropout({ rate: 0.3, name: 'dropout' }).apply(dense1);
    const output = tf.layers.dense({ units: 2, activation: 'softmax', name: 'alcohol_prediction' }).apply(dropout);
  ...
  async checkAlcoholLLM() {
    try {
      ...
      const hasAlcoholModel = models.some(m => m.name.includes('goldentime-alcohol'));
  ...
  async detectAlcohol(biometricData, userBaseline) {
    try {
      ...
      const deviationAnalysis = await this.analyzeBaselineDeviation(biometricData, userBaseline);
      
      // 2단계: Time-series Transformer 예측
      const transformerPrediction = await this.runTransformerAnalysis(biometricData);
      
      // 3단계: LLM 기반 종합 분석
      const llmAnalysis = await this.runLLMAnalysis(biometricData, userBaseline, deviationAnalysis);
      ...
      const explanation = this.generateExplanation(finalResult, biometricData, userBaseline);

      return {
        detected: finalResult.alcohol_detected,
        confidence: confidence,
        severity: finalResult.alcohol_level, // 'none', 'mild', 'moderate', 'severe'
  ...
  async analyzeBaselineDeviation(biometricData, userBaseline) {
    ...
    if (userBaseline.hr_mean) {
      deviations.hr_change = ((biometricData.heartRate - userBaseline.hr_mean) / userBaseline.hr_mean) * 100;
    }

    // HRV 변화율 계산  
    if (userBaseline.hrv_mean && biometricData.hrv) {
      deviations.hrv_change = ((biometricData.hrv - userBaseline.hrv_mean) / userBaseline.hrv_mean) * 100;
    }

    // 스트레스 변화
    if (userBaseline.stress_mean) {
      deviations.stress_change = biometricData.stressLevel - userBaseline.stress_mean;
    }

    // 체온 변화
    if (userBaseline.temp_mean && biometricData.bodyTemperature) {
      deviations.temp_change = biometricData.bodyTemperature - userBaseline.temp_mean;
    }
    ...
    const flags = {
      ...
      stress_elevated: deviations.stress_change > this.thresholds.stress_increase,
   ...
   * LLM 기반 종합 분석
   */
  async runLLMAnalysis(biometricData, userBaseline, deviationAnalysis) {
    const prompt = `
음주 상태 분석 요청:
...
위 생체데이터를 종합하여 음주 상태를 분석해주세요.
응답 형식: "음주상태: [없음/경미/중등도/심각] | 근거: [상세분석] | 신뢰도: [0.0-1.0]"
    `;
  ...
  parseAlcoholAnalysis(analysis) {
    ...
    if (analysis.includes('심각')) {
      result.alcohol_level = 'severe';
      result.confidence = 0.9;
    } else if (analysis.includes('중등도')) {
      result.alcohol_level = 'moderate'; 
      result.confidence = 0.8;
    } else if (analysis.includes('경미')) {
      result.alcohol_level = 'mild';
  ...
  integratePredictions({ deviation, transformer, llm }) {
    ...
    const llmScore = llm.alcohol_level === 'severe' ? 0.9 : 
                     llm.alcohol_level === 'moderate' ? 0.7 :
                     llm.alcohol_level === 'mild' ? 0.5 : 0.1;
    ...
    // 임계값 기반 최종 판정
    const alcohol_detected = combinedScore > 0.6;
    const alcohol_level = combinedScore > 0.8 ? 'severe' :
                         combinedScore > 0.65 ? 'moderate' :
                         combinedScore > 0.5 ? 'mild' : 'none';
  ...
  generateExplanation(result, biometricData, userBaseline) {
    ...
    if (result.evidence.baseline_flags.hr_elevated) {
      explanations.push(`심박수가 평상시보다 ${((biometricData.heartRate - userBaseline.hr_mean) / userBaseline.hr_mean * 100).toFixed(1)}% 증가`);
    }
  ...
  generateRecommendation(result) {
    ...
    switch (result.alcohol_level) {
      case 'severe':
        recommendations.push('즉시 응급의료진 연락');
        recommendations.push('알코올 중독 위험 - 전문의 상담 필요');
        recommendations.push('안전한 장소에서 휴식');
        break;
        
      case 'moderate':
        recommendations.push('운전 및 위험 활동 금지');
        recommendations.push('수분 섭취 및 휴식 권장');
        recommendations.push('지속적인 생체신호 모니터링');
        break;
        
      case 'mild':
        recommendations.push('주의깊은 관찰 필요');
        recommendations.push('추가 음주 자제 권고');
        break;
    }
  ...
  async updateUserBaseline(userId, biometricData) {
    try {
      ...
      const isNormalState = biometricData.heartRate >= 50 && biometricData.heartRate <= 100 &&
