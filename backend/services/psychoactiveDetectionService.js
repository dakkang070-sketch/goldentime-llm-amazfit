/**
 * 향정신성약물 탐지 서비스
 * Amazfit 생체데이터 기반 향정신성약물 사용 상태 실시간 분석
...
class PsychoactiveDetectionService {
  constructor() {
    ...
    this.psychoactivePatterns = {
      benzodiazepines: { // 벤조디아제핀 (자낙스, 아티반 등)
  ...
  createPsychoactiveTransformerModel() {
    ...
    const temporalEncoded = tf.layers.dense({
      units: 128,
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // Layer normalization
      const norm = tf.layers.layerNormalization({ name: `psychoactive_norm_${i}` });
      attention = norm.apply(tf.layers.add().apply([temporalEncoded, attention]));
    ...
    const weightedPooled = tf.layers.dot({ axes: [1, 1], name: 'weighted_temporal_pooling' })
      .apply([timeWeights.apply(input), reshapedAttention]);
    
    const flattened = tf.layers.flatten({ name: 'flatten_pooled' }).apply(weightedPooled);
  ...
  async checkPsychoactiveLLM() {
    try {
      ...
      const hasPsychoactiveModel = models.some(m => m.name.includes('goldentime-psychoactive'));
  ...
  async detectPsychoactive(biometricData, userBaseline) {
    try {
      ...
      // 1단계: CNS 억제 패턴 분석
      const cnsDepressionAnalysis = await this.analyzeCNSDepression(biometricData, userBaseline);
      
      // 2단계: 점진적 변화 추세 분석
      const trendAnalysis = await this.analyzeGradualTrends(biometricData, userBaseline);
      
      // 3단계: Transformer 기반 패턴 분류
      const transformerPrediction = await this.runPsychoactiveTransformerAnalysis(biometricData);
      
      // 4단계: 인지/움직임 억제 분석
      const cognitiveMotorAnalysis = await this.analyzeCognitiveMotorSuppression(biometricData, userBaseline);
      
      // 5단계: LLM 기반 종합 해석
      const llmAnalysis = await this.runPsychoactiveLLMAnalysis(
        biometricData, userBaseline, cnsDepressionAnalysis, trendAnalysis
      );
      ...
      return {
        ...
        explanation: this.generatePsychoactiveExplanation(finalResult, biometricData, userBaseline),
  ...
  async analyzeCNSDepression(biometricData, userBaseline) {
    ...
    // 1. 심박수 점진적 감소 패턴 분석
    indicators.hr_gradual_decline = this.analyzeGradualHeartRateDecline(recentSequence, userBaseline);
    
    // 2. 움직임 활동성 억제 분석
    indicators.movement_suppression = this.analyzeMovementSuppression(recentSequence, userBaseline);
    
    // 3. 호흡 억제 패턴 분석
    indicators.respiratory_depression = this.analyzeRespiratoryDepression(recentSequence, userBaseline);
    
    // 4. 반응성 저하 분석 (간접적)
    indicators.responsiveness_decline = this.analyzeResponsivenessDecline(recentSequence);
    
    // 5. 체온 조절 기능 변화
    indicators.thermoregulation_impact = this.analyzeThermoregulationImpact(recentSequence, userBaseline);

    // CNS 억제 종합 점수 계산
    const scores = [
      indicators.hr_gradual_decline.score * 0.25,      // 25%
      indicators.movement_suppression.score * 0.30,    // 30% (주요 지표)
      indicators.respiratory_depression.score * 0.20,  // 20%
      indicators.responsiveness_decline.score * 0.15,  // 15%
      indicators.thermoregulation_impact.score * 0.10  // 10%
    ];
    ...
    return {
      ...
      indicators,
      severity: cns_depression_score > 0.7 ? 'severe' : 
                cns_depression_score > 0.5 ? 'moderate' : 
                cns_depression_score > 0.3 ? 'mild' : 'none',
  ...
  analyzeMovementSuppression(sequence, userBaseline) {
    ...
    // 정적 상태 비율 계산
    const stationaryRatio = movementData.reduce((sum, m) => sum + m.stationaryTime, 0) / movementData.length;
    ...
    const changeReductionScore = Math.max(0, (baselineChangeFreq - changeFrequency) / baselineChangeFreq);
    ...
    return {
      ...
      suppressed: stationaryRatio > 0.8 && changeFrequency < baselineChangeFreq * 0.5
    };
  }
  ...
  analyzeRespiratoryDepression(sequence, userBaseline) {
    const respiratoryValues = sequence.map(d => d.respiratoryRate).filter(r => r > 0);
    if (respiratoryValues.length < 3) return { score: 0, depressed: false };

    const baselineRespRate = userBaseline.resp_rate_mean || 16;
    const currentAvgRespRate = respiratoryValues.reduce((sum, r) => sum + r, 0) / respiratoryValues.length;
    
    // 호흡수 감소율 계산
    const depressionPercentage = (baselineRespRate - currentAvgRespRate) / baselineRespRate;
  ...
  analyzeResponsivenessDecline(sequence) {
    ...
    const hrValues = sequence.map(d => d.heartRate).filter(hr => hr > 0);
    const stressValues = sequence.map(d => d.stressLevel).filter(s => s >= 0);
    ...
    // 심박수 변동성 (HRV 대체 지표)
    const hrVariability = hrValues.length > 1 ? this.calculateVariance(hrValues) : 0;
    const hrCV = hrValues.length > 1 ? Math.sqrt(hrVariability) / (hrValues.reduce((sum, hr) => sum + hr, 0) / hrValues.length) : 0;
    
    // 스트레스 반응성
    const stressVariability = stressValues.length > 1 ? this.calculateVariance(stressValues) : 0;
  ...
  analyzeThermoregulationImpact(sequence, userBaseline) {
    ...
    const currentAvgTemp = tempValues.reduce((sum, t) => sum + t, 0) / tempValues.length;
    ...
    const coolingScore = coolingEffect > 0 ? Math.min(coolingEffect * 2, 0.5) : 0;
  ...
  async analyzeGradualTrends(biometricData, userBaseline) {
    const extendedSequence = await this.getRecentBiometricSequence(biometricData.userId, 30);
    ...
    // 심박수 추세
    const hrValues = extendedSequence.map(d => d.heartRate).filter(hr => hr > 0);
    trends.hr_trend = {
      slope: this.calculateLinearSlope(timePoints.slice(0, hrValues.length), hrValues),
      r_squared: this.calculateRSquared(timePoints.slice(0, hrValues.length), hrValues)
    };
    
    // 움직임 활동도 추세 (수치화)
    const movementScores = extendedSequence.map(d => 
      d.movementStatus === 'stationary' ? 0 : 
      d.movementStatus === 'walking' ? 1 : 2
    );
    ...
    // 스트레스/각성 수준 추세
    const stressValues = extendedSequence.map(d => d.stressLevel).filter(s => s >= 0);
    trends.stress_trend = {
      slope: this.calculateLinearSlope(timePoints.slice(0, stressValues.length), stressValues),
      r_squared: this.calculateRSquared(timePoints.slice(0, stressValues.length), stressValues)
    };
  ...
  async runPsychoactiveTransformerAnalysis(biometricData) {
    ...
    try {
      const sequenceData = await this.preparePsychoactiveSequenceData(biometricData);
      ...
      const categories = ['normal', 'benzodiazepines', 'barbiturates', 'z_drugs', 'antipsychotics'];
  ...
  async preparePsychoactiveSequenceData(currentData) {
    try {
      ...
      return recentData.reverse().map(data => [
        ...
        data.sleepStatus === 'awake' ? 0 : data.sleepStatus === 'light_sleep' ? 1 : 2 // 각성 수준
      ]);
  ...
  async analyzeCognitiveMotorSuppression(biometricData, userBaseline) {
    ...
    const cognitiveIndicators = {
      reaction_sluggishness: this.assessReactionSluggishness(biometricData, userBaseline),
    ...
    const suppression_score = Object.values(cognitiveIndicators)
      .reduce((sum, indicator) => sum + indicator.score, 0) / 4;

    return {
      suppression_score,
      cognitive_indicators: cognitiveIndicators,
      motor_suppression_level: suppression_score > 0.7 ? 'severe' : 
                              suppression_score > 0.5 ? 'moderate' : 
                              suppression_score > 0.3 ? 'mild' : 'none'
    };
  }
   ...
   * LLM 기반 향정신성약물 분석
   */
  async runPsychoactiveLLMAnalysis(biometricData, userBaseline, cnsAnalysis, trendAnalysis) {
    const prompt = `
향정신성약물 사용 상태 분석 요청:
...
위 생체데이터와 패턴을 분석하여 향정신성약물 사용 가능성과 분류를 판단해주세요.
응답 형식: "향정신성약물: [없음/벤조디아제핀/바르비튜레이트/Z약물/항정신병약] | 심각도: [없음/경미/중등도/심각] | 근거: [상세분석] | 신뢰도: [0.0-1.0]"
    `;
  ...
  calculateRSquared(xValues, yValues) {
    ...
    const intercept = yMean - slope * (xValues.reduce((sum, x) => sum + x, 0) / xValues.length);
  ...
  assessReactionSluggishness(biometricData, userBaseline) {
    ...
    const sluggishness = Math.max(0, (baselineStress - currentStress) / baselineStress);
    return { score: Math.min(sluggishness * 2, 1.0) };
  }

  assessMotorCoordination(biometricData) {
    // 움직임 패턴의 일관성 기반 평가
    const movementConsistency = biometricData.movementStatus === 'stationary' ? 0.7 : 0.3;
  ...
  assessAttentionSpan(biometricData, userBaseline) {
    ...
    const attentionDeficit = Math.max(0, (baselineHRV - currentHRV) / baselineHRV);
  ...
  assessProcessingSpeed(biometricData, userBaseline) {
    ...
    const processingSlowdown = Math.max(0, (baselineHR - currentHR) / baselineHR);
    return { score: Math.min(processingSlowdown * 2, 1.0) };
  }

  calculatePatternConsistency(trends) {
    const rSquaredValues = Object.values(trends).map(t => t.r_squared).filter(r => r > 0);
  ...
  integratePsychoactivePredictions({ cns_depression, trend, transformer, cognitive_motor, llm }) {
    const weights = {
    ...
    // 최종 판정 (향정신성약물은 보수적 기준)
    const psychoactive_detected = combinedScore > 0.65;
    
    // 약물 분류 결정
    let suspected_category = 'none';
    if (psychoactive_detected) {
      if (transformer.drug_category !== 'normal' && transformer.drug_category !== 'unknown') {
        suspected_category = transformer.drug_category;
      } else if (llm.drug_category !== 'none' && llm.drug_category !== 'unknown') {
    ...
    // 심각도 결정
    const severity_level = combinedScore > 0.85 ? 'severe' :
                          combinedScore > 0.75 ? 'moderate' :
                          combinedScore > 0.6 ? 'mild' : 'none';
  ...
  generatePsychoactiveExplanation(result, biometricData, userBaseline) {
    ...
    const categoryExplanation = result.suspected_category === 'benzodiazepines' ? '벤조디아제핀계 약물 사용 패턴' :
                               result.suspected_category === 'barbiturates' ? '바르비튜레이트계 약물 사용 패턴' :
                               result.suspected_category === 'z_drugs' ? 'Z-약물(수면제) 사용 패턴' :
                               result.suspected_category === 'antipsychotics' ? '항정신병약물 사용 패턴' :
                               '향정신성약물 사용 패턴';
  ...
  generatePsychoactiveRecommendation(result) {
    ...
    switch (result.severity_level) {
      case 'severe':
        recommendations.push('🚨 즉시 응급의료진 호출');
        recommendations.push('🫁 호흡 상태 지속 모니터링 필수');
        recommendations.push('🏥 중독치료 전문병원 이송');
        recommendations.push('💊 해독 치료 프로토콜 적용');
        recommendations.push('👁️ 의식수준 면밀 관찰');
        break;

      case 'moderate':
        recommendations.push('⚠️ 의료진 상담 필요');
        recommendations.push('🔍 독성학 검사 권장');
        recommendations.push('🏠 안전한 환경에서 관찰');
        recommendations.push('📞 정신건강 전문의 연락');
        break;

      case 'mild':
        recommendations.push('👀 지속적인 관찰 필요');
        recommendations.push('💬 약물 사용 상담');
        recommendations.push('📊 추가 모니터링 실시');
        break;
    }
